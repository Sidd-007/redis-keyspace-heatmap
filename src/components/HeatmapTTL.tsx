...
interface HeatmapTTLProps {
    data: PrefixAgg[];
    ttlBuckets: number[];
    onCellClick?: (prefix: string, ttlBucket: string) => void;
    colorBy: 'count' | 'bytes';
}

export function HeatmapTTL({ data, ttlBuckets, onCellClick, colorBy = 'count' }: HeatmapTTLProps) {
    const [hoveredCell, setHoveredCell] = useState<{ prefix: string; ttlBucket: string; value: number; originalValue: number } | null>(null);
    ...
    const bucketLabels = ttlBuckets.map(bucket =>
        bucket === 0 ? 'Persistent' : formatDuration(bucket * 1000)
    );
    ...
    const filteredAndSortedData = useMemo(() => {
        ...
        const withTotals = filtered.map(prefix => {
            const totalCount = Object.values(prefix.ttlHist).reduce((sum, count) => sum + count, 0);
    ...
    const heatmapData: Array<{ prefix: string; ttlBucket: string; value: number; originalValue: number }> = [];

    paginatedData.forEach(prefix => {
        bucketLabels.forEach((label, index) => {
            const bucketKey = Object.keys(prefix.ttlHist)[index];
            const value = prefix.ttlHist[bucketKey] || 0;
        ...
        <div className="h-full flex flex-col">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 max-w-xs">
                            <input
                                ...
                                className="w-full px-2 py-1 border border-input rounded text-xs focus:outline-none focus:ring-1 focus:ring-green-500 bg-background text-foreground"
                            />
                        </div>
                    </div>
            ...
            <div className="flex-1 bg-card border rounded-lg shadow-sm overflow-hidden">
                <div className="p-3 pb-1 flex-shrink-0">
                    <h3 className="text-sm font-bold text-foreground mb-2">
                        TTL Heatmap ({currentColorBy === 'count' ? 'Key Count' : 'Memory Usage'})
                    </h3>
                </div>
                                    ...
                                    <TableRow key={prefix.prefix}>
                                        ...
                                        {bucketLabels.map((label) => {
                                            const cellData = heatmapData.find(d =>
                                                d.prefix === prefix.prefix && d.ttlBucket === label
                                            );
                ...
                <div className="p-3 pt-1 border-t bg-muted/20 flex-shrink-0">
                    <div className="flex items-center justify-center space-x-2">
                        ...
                        <div className="flex space-x-1">
                            ...
                            <div className="w-2 h-2 bg-green-100 dark:bg-green-900 rounded"></div>
                            <div className="w-2 h-2 bg-green-200 dark:bg-green-800 rounded"></div>
                            <div className="w-2 h-2 bg-green-300 dark:bg-green-700 rounded"></div>
                            <div className="w-2 h-2 bg-green-400 dark:bg-green-600 rounded"></div>
                        </div>
            ...
            {hoveredCell && mounted && (
                <div className="fixed bg-popover text-popover-foreground p-2 rounded text-xs pointer-events-none z-50 shadow-lg border"
                    ...
                    <div className="text-muted-foreground">TTL: {hoveredCell.ttlBucket}</div>
